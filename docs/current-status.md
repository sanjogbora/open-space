...
- Studio Objects now labels each object row as needs top hide, movement role, hidden, top hidden, or object ready beside the triangle count.
...
- Studio Rooms and Views now label list rows with plain setup states such as needs view, needs map area, inside blocker, outside walk area, room linked, or ready.
...
- Studio Publish now shows a success note that distinguishes client-live publishes from internal draft versions that are not live yet.
- Studio Publish client-test actions now open the exact published draft URL when a draft version exists but no live link has been selected.
- Studio Publish version rows now label non-active bundles as Not live beside the delivery-mode pill.
...
- Studio Source QA, Viewer QA, and publish readiness now surface partial default-scene exports and non-triangle geometry modes, so missing model parts and unpredictable click/collision surfaces get a visible source-review path.
- Viewer QA now surfaces physical glass or transmission materials as a visible material-review reminder, so users compare window quality and mobile cost after optimization without treating valid glass as a publish blocker.
...
- Studio Bake now shows failed or blocked bake jobs as failure cards and can copy a failure report with job stages, preflight state, settings, and retry guidance.
